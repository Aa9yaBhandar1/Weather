...
const searchBoxLanding = document.querySelector("#search-city-landingPage");
const searchBtnLanding = document.querySelector("#search-btn-landingPage");
const searchBoxMain = document.querySelector("#search-city-mainPage input");
const searchBtnMain = document.querySelector("#search-btn-mainPage");
...
async function checkWeather(city) {
  ...
  document.querySelector(".temp-max").innerHTML =
    Math.trunc(data.main.temp_max) + "°C";
  document.querySelector(".temp-min").innerHTML =
    Math.trunc(data.main.temp_min) + "°C";
...
searchBtnLanding.addEventListener("click", () => {
  const city = searchBoxLanding.value.trim();
  if (city) {
    checkWeather(city);
    getForecast(city);
    //showMainPage();
    togglePage();
    searchBoxLanding.value = "";
  } else {
    alert("Please enter a city name");
  }
});
searchBtnMain.addEventListener("click", () => {
  const city = searchBoxMain.value.trim();
  if (city) {
    checkWeather(city);
    getForecast(city);
    //showMainPage();
...
document
  .querySelector("#search-city-landingPage")
...
document
  .querySelector("#search-city-mainPage input")
  .addEventListener("keypress", function (event) {
    const city = searchBoxMain.value.trim();
    if (event.key === "Enter") {
      checkWeather(city);
...
function updateForecast(forecastData) {
  ...
  forecastData.forEach((data, index) => {
    if (weatherBoxes[index]) {
      ...
      // Update the elements
      weatherBoxes[index].querySelector(".forecast-day").innerText =
        formattedDate;
      weatherBoxes[index].querySelector(".forecast-temperature").innerText =
        data.main.temp + "°C";
      ...
      forecastData.forEach((data, index) => {
        if (weatherBoxes[index]) {
          ...
          // Update weather icon
          const forecastIcons =
            weatherBoxes[index].querySelector(".forecast-icons2");
          if (forecastIcons) {
